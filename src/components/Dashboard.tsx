  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-slate-900 border border-slate-700/50 p-3 rounded-lg shadow-xl shrink-0 min-w-[150px]">
        <p className="text-white font-semibold text-sm mb-2 pb-2 border-b border-slate-700/50">
...
import { adSetPerformance, spendTrendData } from "../data/mockData";
...
export function Dashboard() {
  ...
  const [dailyTrendDimension, setDailyTrendDimension] = useState<
    "total" | "platform" | "campaign" | "adCode"
  >("total");
  const [rawUploadData, setRawUploadData] = useState<any[]>(() =>
    adSetPerformance.map((row) => ({ ...row, _derived: { ...row } })),
  );
  const [rawHeaders, setRawHeaders] = useState<string[]>(
    Object.keys(adSetPerformance[0] || {}),
  );
  ...
  const filteredRawRows = useMemo(() => {
    return rawUploadData.filter((row) => {
      ...
      const matchPlatform =
        ...
        selectedPlatforms.includes(camp.platform);
      const matchAdSet =
        adSetSearch === "" ||
        (camp.adSet &&
  ...
  const totalWalkins = filteredAdSets.reduce(
    (acc, curr) => acc + curr.walkins,
    0,
  );
  const totalBookings = filteredAdSets.reduce(
  ...
  const funnel = [
    ...
    {
      stage: "Appointments",
      count: filteredAdSets.reduce((acc, curr) => acc + curr.appointments, 0),
    },
  ...
  const sources = Object.entries(sourceDataMap).map(([name, value]) => ({
    name,
    value,
  }));
  ...
  if (lostReasons.length === 0 && totalLeads > 0) {
    ...
    lostReasons = [
      ...
      { reason: "Not Interested", count: Math.floor(totalLeads * 0.2) },
  ...
  const projectDataMap: Record<
    string,
    { leads: number; siteVisits: number; closed: number }
  > = {};
  filteredAdSets.forEach((c) => {
    if (!projectDataMap[c.project])
      projectDataMap[c.project] = { leads: 0, siteVisits: 0, closed: 0 };
  ...
  const projects = Object.entries(projectDataMap).map(([region, metrics]) => ({
    region,
    ...metrics,
  }));
  ...
  const trendDataMap: Record<
    string,
    { spend: number; leads: number; walkins: number }
  > = {};
  ...
  const { dailyLeadFlowChartData, dailyLeadFlowKeys } = useMemo(() => {
    if (dailyTrendDimension === "total") {
      return {
        dailyLeadFlowChartData: trendChartData,
        dailyLeadFlowKeys: ["leads", "walkins"],
      };
    }
      ...
      Papa.parse(file, {
        complete: (results) => {
          ...
          try {
            ...
            const isRawLeadData = headers.some((h) =>
              ["Enquiry ID", "Date of Enquiry", "Enquiry Status"].includes(h),
            );

            const getValRawWrapper = (row: any, keys: string[]) => {
              const lowerKeys = keys.map((k) => k.toLowerCase());
              const matchingKey = Object.keys(row).find((k) =>
                lowerKeys.includes(k.toLowerCase().trim()),
              );
            ...
            const processedRawRows = validRows.map((row) => {
              ...
              if (isRawLeadData) {
                ...
                const dateVal =
                  row["Date of Enquiry"] || row["Last Modified Date"] || "";
                if (dateVal) {
            ...
            if (isRawLeadData) {
              ...
              processedRawRows.forEach((row, i) => {
                ...
                if (
                  row["Visit AP Count"] === "1" ||
                  row["AP Proposed Counter"] === "1" ||
                  row["Appointment Done"] === "Yes" ||
                  enqStatus.includes("appointment")
                ) {
                  agg.appointments += 1;
                  agg.qualified += 1;
                }
                ...
                if (
                  row["Walk-in Counter"] === "1" ||
                  row["Site Visit Done"] === "1" ||
                  row["Site visit requested"] === "1" ||
                  enqStatus.includes("visit") ||
                  String(row["Media Type"]).toLowerCase().includes("walk") ||
                  enqStatus.includes("walkin")
                ) {
                  agg.walkins += 1;
                  agg.appointments += 1;
                  agg.qualified += 1;
                ...
                if (
                  bookStatus.includes("booking done") ||
                  enqStatus.includes("booking") ||
                  enqStatus.includes("offer")
                ) {
                  agg.bookings += 1;
                }
              });
            ...
            if (mappedData.length > 0) {
              ...
              setToastMessage(
                "File parsed, but no relevant mapping keys (Leads/Enquiry IDs) found.",
              );
            }
          } catch (e: any) {
    ...
    <main className="flex-1 flex flex-col min-w-0">
      ...
      <header className="h-16 bg-white border-b border-slate-200 px-4 flex items-center justify-between shrink-0 relative z-20">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-medium text-slate-500 font-medium truncate max-w-[120px] sm:max-w-none">
            Dashboard
          </span>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <div className="flex-1 p-4 sm:p-6 lg:p-8 space-y-6 overflow-y-auto bg-white">
          ...
          <div className="flex flex-col gap-6">
              ...
              <Card className="overflow-hidden flex flex-col">
                <div className="bg-slate-50/80 px-6 py-4 border-b border-slate-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-slate-500">
                    Platform & Ad-Set Performance (Drilldown)
                  </h3>
            ...
            {activeTableView === "walkin" && (
              <WalkinLostTable data={filteredAdSets} />
            )}
            {activeTableView === "lostreason" && (
              <LostReasonTable data={filteredRawRows} />
            )}
            {activeTableView === "raw" && (
              <RawDataTable data={filteredRawRows} headers={rawHeaders} />
            )}
          </div>
        </div>
      </div>
...
function KpiCard({
  ...
  return (
    <div className="bg-white p-4 sm:p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col">
      ...
      <div className="text-2xl sm:text-3xl font-bold mt-1 text-slate-900">
        {value}
      </div>
      <div
        className={`text-xs font-bold flex items-center mt-3 ${
          changeType === "positive"
            ? "text-emerald-600"
            : changeType === "negative"
              ? "text-rose-600"
